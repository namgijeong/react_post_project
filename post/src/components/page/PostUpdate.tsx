...
import { makePutRequestBaseAndExecuteAxios } from '../../axios/axiosHook';
...
function PostUpdate() {
   ...
   const { params, goToDetailPost, queryStrings } = useRouter();
   ...
   const registerMutation = useMutationReactQuery<InputPost, Result | null>(['posts'], updatePost);
   ...
   const updateClick = async (inputContent: InputPost) => {
      // console.log('등록완료');
      registerMutation.mutate({
         title: inputContent.title,
         writer: inputContent.writer,
         content: inputContent.content,
      });
      reset();
      await goToDetailPost(detailId);
   };
   ...
   return (
      <div css={DetailDivStyle}>
         ...
         <Box
            ...
            <Button sx={{ width: 150, height: 50 }} variant="contained" onClick={handleSubmit(updateClick)}>
               작성완료
            </Button>
         </Box>
      </div>
