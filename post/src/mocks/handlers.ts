...
import {
  getPostsData, getCommentsData, changePostsData, changeCommentsData,
} from '../faker/storeFakerData';
...
export const handlers = [
  ...
  http.post('/post', async ({ params, request }) => {
    ...
    const bodyPost = {
      ...bodyData, id: faker.number.int({ min: 1, max: 999 }), readCount: 0, likeCount: 0, regDate: faker.date.recent().toString(),
    };
    // 데이터 배열과 연동
    let posts: Array<Post> = postsData;
  ...
  http.put('/post/:id', async ({ params, request }) => {
    ...
    let posts: Array<Post> = postsData;
    // 객체의 복사와 일부 속성 덮어쓰기기
    const newArr = posts.map((item) => (item.id === Number(id) ? {
      ...item, title: bodyData.title, writer: bodyData.writer, content: bodyData.content,
    } : item));
  ...
  http.delete('/post/:id', async ({ params, request }) => {
    ...
    let posts: Array<Post> = postsData;
    // filter는 콜백 함수가 true를 반환하는 원소만 남기는 함수
    const newArr = posts.filter((item) => item.id !== Number(id));
  ...
  http.post('/comment/:postId', async ({ params, request }) => {
    ...
    const bodyPost = { ...bodyData, id: faker.number.int({ min: 1, max: 999 }), regDate: faker.date.recent().toString() };
    console.log(`msw bodyData : ${bodyData}`);
  ...
  http.delete('/comment/:postId/:id', async ({ params, request }) => {
    ...
    let comments: Array<MyComment> = commentsData;
    // filter는 콜백 함수가 true를 반환하는 원소만 남기는 함수
    const newArr = comments.filter((item) => item.id !== Number(id));
